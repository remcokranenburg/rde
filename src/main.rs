...
use gtk4::{gio, glib, prelude::*, ListItemFactory};
...
impl SimpleComponent for App {
    ...
    view! {
            ...
            gtk::Box {
                ...
                gtk::Box {
                    ...
                    gtk::MenuButton {
                        set_label: "Start",
                        #[wrap(Some)]
                        set_popover = &gtk::Popover::new() {
                            set_child: Some(&popover_child),
                            set_has_arrow: false,
        ...
        popover_child = gtk::Box {
            set_orientation: gtk::Orientation::Horizontal,
            append = &gtk::PopoverMenu::from_model(Some(&start_menu)),

            gtk::Label {
                set_label: {
                    let default_label = glib::GString::from("Redmond Desktop Environment");
                    let key_file = glib::KeyFile::new();

                    &key_file.load_from_file("/etc/os-release", glib::KeyFileFlags::NONE).and_then(|_| {
                        Ok(key_file.string("", "PRETTY_NAME").unwrap_or(default_label.clone()))
